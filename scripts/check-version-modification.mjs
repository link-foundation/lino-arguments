 ...
 *
 * Usage:
 *   node scripts/check-version-modification.mjs [--rust-root <path>]
 *
...
function getCargoTomlDiff() {
  ...
  const diff = exec(`git diff origin/${baseRef}...HEAD -- ${CARGO_TOML}`);
  return diff;
}
 ...
 */
function main() {
  console.log(`Checking for manual version modifications in ${CARGO_TOML}...\n`);
  ...
  if (!diff) {
    console.log(`No changes to ${CARGO_TOML} detected.`);
  ...
  if (hasVersionChange(diff)) {
    console.error(`Error: Manual version change detected in ${CARGO_TOML}!\n`);
    ...
    console.error(`  git checkout origin/main -- ${CARGO_TOML}`);
    process.exit(1);
  }

  console.log(`${CARGO_TOML} was modified but version field was not changed.`);
  console.log('Version check passed.');
  process.exit(0);
}
