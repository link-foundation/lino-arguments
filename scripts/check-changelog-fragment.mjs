 ...
 *
 * Usage: node scripts/check-changelog-fragment.mjs [--rust-root <path>]
 *
...
function isSourceFile(filePath) {
  // Get the rust root prefix for path matching
  const prefix = rustRoot === '.' ? '' : `${rustRoot}/`;

  // Source files that require changelog fragments
  const sourcePatterns = [
    new RegExp(`^${prefix}src/`),
    new RegExp(`^${prefix}tests/`),
    new RegExp(`^scripts/`),
    new RegExp(`^${prefix}Cargo\\.toml$`),
  ];
...
function isChangelogFragment(filePath) {
  ...
  // Support both root-level and rust/ subdirectory paths
  const changelogDirPattern = rustRoot === '.'
    ? /^changelog\.d\//
    : new RegExp(`^${rustRoot}/changelog\\.d/`);

  return (
    changelogDirPattern.test(filePath) &&
...
function checkChangelogFragment() {
  ...
  if (sourceChangedCount > 0 && fragmentAddedCount === 0) {
    console.error(
      `::error::No changelog fragment found in this PR. Please add a changelog entry in ${CHANGELOG_DIR}/`
    );
    console.error('');
    console.error('To create a changelog fragment:');
    console.error(
      `  Create a new .md file in ${CHANGELOG_DIR}/ with your changes`
    );
    console.error('');
    console.error(`See ${CHANGELOG_DIR}/README.md for more information.`);
    process.exit(1);
  }
